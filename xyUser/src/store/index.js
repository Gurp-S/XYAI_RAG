...
/** 将后端对话历史原始数据标准化为消息数组。idPrefix 用于历史消息去重（如 "old_"） */
function parseConversationHistory(rawData, idPrefix = "") {
  return (Array.isArray(rawData) ? rawData : [])
    .map((msg) => {
      const msgs = [];
      if (msg.userMessage) {
        msgs.push({
          id: `${idPrefix}${msg.chatMessageId || Date.now()}`,
          role: "user",
...
const BG_OPACITY_KEY = "userBackgroundOpacity";
const BORDER_CONTRAST_KEY = "xy-border-contrast";
const CUSTOM_PRIMARY_KEY = "xy-custom-primary";
const CUSTOM_BORDER_KEY = "xy-custom-border";
...
function clampBorderContrast(value) {
  const num = Number(value);
  if (!Number.isFinite(num)) return 100;
  return Math.min(100, Math.max(0, Math.round(num)));
}

function readBorderContrast() {
  const raw = localStorage.getItem(BORDER_CONTRAST_KEY);
  if (raw === null) return 100;
  return clampBorderContrast(raw);
}

function readCustomPrimary() {
  try {
    const val = localStorage.getItem(CUSTOM_PRIMARY_KEY);
    return val && /^#[0-9a-f]{6}$/i.test(val) ? val : "";
  } catch {
    return "";
  }
}

function readCustomBorder() {
  try {
    const val = localStorage.getItem(CUSTOM_BORDER_KEY);
    return val && /^#[0-9a-f]{6}$/i.test(val) ? val : "";
  } catch {
    return "";
  }
...
function ensureSessionExpiryTimer(store) {
  ...
  sessionExpiryTimer = setTimeout(async () => {
    ...
    try {
      ...
      if (newToken) {
        ensureSessionExpiryTimer(store);
        return;
      }
    } catch (error) {
      console.warn("Token refresh failed in expiry timer:", error);
    }
...
function readBackgroundOpacity() {
  const raw = localStorage.getItem(BG_OPACITY_KEY);
  if (raw === null) return 1;
  return clampBackgroundOpacity(raw);
}
...
/**
 * Parse a CSS color value string into its RGB components.
 * Handles both rgba(r,g,b,a) and color(srgb r g b / a) formats.
 * Returns null if the format is not recognized.
 */
function getSurfaceBaseColors(darkMode, hasBg) {
  if (darkMode) {
    if (hasBg) {
      return { solid: "9,15,28", soft: "9,15,28", glassStrong: "9,15,28", glassSoft: "9,15,28", botMsg: "9,15,28" };
    }
    return { solid: "14,23,38", soft: "14,23,38", glassStrong: "14,23,38", glassSoft: "14,23,38", botMsg: "16,26,43" };
  }
  if (hasBg) {
    return { solid: "255,255,255", soft: "244,249,255", glassStrong: "255,255,255", glassSoft: "248,252,255", botMsg: "247,251,255" };
  }
  return { solid: "255,255,255", soft: "244,249,255", glassStrong: "255,255,255", glassSoft: "247,251,255", botMsg: "248,252,255" };
}

const SURFACE_STYLE_ID = "xy-surface-opacity";

function injectSurfaceOpacityStyle(darkMode, hasBg, opacity) {
  const el =
    document.getElementById(SURFACE_STYLE_ID) ||
    (() => {
      const e = document.createElement("style");
      e.id = SURFACE_STYLE_ID;
  ...
  const c = getSurfaceBaseColors(darkMode, hasBg);
  el.textContent = [
    `body{--surface-solid:rgba(${c.solid},${opacity})!important;}`,
    `body{--surface-soft:rgba(${c.soft},${opacity})!important;}`,
    `body{--glass-strong:rgba(${c.glassStrong},${opacity})!important;}`,
    `body{--glass-soft:rgba(${c.glassSoft},${opacity})!important;}`,
    `body{--bot-msg:rgba(${c.botMsg},${opacity})!important;}`,
  ].join("");
}

const BORDER_CONTRAST_STYLE_ID = "xy-border-contrast";

function injectBorderContrastStyle(contrast) {
  const el =
    document.getElementById(BORDER_CONTRAST_STYLE_ID) ||
    (() => {
      const e = document.createElement("style");
      e.id = BORDER_CONTRAST_STYLE_ID;
      document.head.appendChild(e);
      return e;
    })();

  // Clear previous override FIRST so getComputedStyle below reads the live
  // theme value (or custom border colour from injectCustomColors), not the
  // stale !important rule from the last call.
  el.textContent = "";

  // Read the current runtime --panel-border RGB (ignore its alpha)
  let borderColor = "";
  try {
    borderColor = getComputedStyle(document.body).getPropertyValue("--panel-border").trim();
  } catch (e) {
    return;
  }
  ...
  // Map 0–100 to alpha 0–1: 100% = fully opaque so borders stay visible
  // even when background transparency is very low.
  const newAlpha = Math.min(1, Math.max(0, contrast / 100));

  // Parse rgba(r,g,b,a) or color-mix(...) output — handle various formats
  // Try rgba(r,g,b,a) first
  const rgbaMatch = borderColor.match(/rgba?\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)(?:\s*,\s*([\d.]+))?\s*\)/);
  if (rgbaMatch) {
    const r = rgbaMatch[1];
    const g = rgbaMatch[2];
    const b = rgbaMatch[3];
    el.textContent = `body{--panel-border:rgba(${r},${g},${b},${newAlpha})!important;}`;
    return;
  }

  // Try color(srgb r g b / a) format
  const colorMatch = borderColor.match(/color\(\s*srgb\s+([\d.]+)\s+([\d.]+)\s+([\d.]+)\s*\/\s*([\d.]+)\s*\)/);
  if (colorMatch) {
  ...
  // Unrecognised format — clear override
  el.textContent = "";
}

/**
 * Apply custom primary and border colors as body inline styles.
 * Runs BEFORE injectBorderContrastStyle so the contrast slider reads the
 * custom border color's RGB when computing its alpha override.
 */
function injectCustomColors(customPrimary, customBorder) {
  const body = document.body;
  if (!body) return;

  if (customPrimary) {
    body.style.setProperty("--primary", customPrimary);
  } else {
...
function applyBodyAppearance(
  ...
  // Clear any inline surface overrides first, so subsequent getComputedStyle
  // reads reflect the current CSS theme state, not previously-modified values
  ...
  for (const v of surfaceVarNames) {
  ...
  body.style.setProperty(
    "--user-bg-opacity",
    String(clampBackgroundOpacity(backgroundOpacity)),
  );
...
export const useUiStore = defineStore("ui", {
  state: () => ({
    ...
    backgroundOpacity: readBackgroundOpacity(),
    borderContrast: readBorderContrast(),
    customPrimary: readCustomPrimary(),
    customBorder: readCustomBorder(),
  ...
  actions: {
    ...
    setTheme(themeName) {
      ...
      const update = () => {
        ...
        // Clear custom colors so the selected theme's palette takes full effect
        this.customPrimary = "";
        this.customBorder = "";
        try {
          localStorage.removeItem(CUSTOM_PRIMARY_KEY);
          localStorage.removeItem(CUSTOM_BORDER_KEY);
        } catch (_) { /* ignore */ }
        this.applyAppearance();
      };
    ...
    setBackgroundOpacity(opacity) {
      this.backgroundOpacity = clampBackgroundOpacity(opacity);
      localStorage.setItem(BG_OPACITY_KEY, String(this.backgroundOpacity));
      this.applyAppearance();
    },
    setBorderContrast(contrast) {
      this.borderContrast = clampBorderContrast(contrast);
      localStorage.setItem(BORDER_CONTRAST_KEY, String(this.borderContrast));
      this.applyAppearance();
    },
    setCustomPrimary(color) {
      const next = /^#[0-9a-f]{6}$/i.test(color) ? color : "";
      this.customPrimary = next;
      try {
        if (next) localStorage.setItem(CUSTOM_PRIMARY_KEY, next);
        else localStorage.removeItem(CUSTOM_PRIMARY_KEY);
      } catch {}
      this.applyAppearance();
    },
    setCustomBorder(color) {
      const next = /^#[0-9a-f]{6}$/i.test(color) ? color : "";
      this.customBorder = next;
      try {
        if (next) localStorage.setItem(CUSTOM_BORDER_KEY, next);
        else localStorage.removeItem(CUSTOM_BORDER_KEY);
      } catch {}
      this.applyAppearance();
    },
    openLogin() {
    ...
    async refreshCurrentConversation() {
      ...
      try {
        ...
        if (response.ok && result?.code === 200) {
          const messages = parseConversationHistory(result.data);
