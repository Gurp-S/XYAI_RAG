            ...
            <transition name="panel-fade">
                <div v-if="showAppearancePanel" ref="appearancePanel" class="appearance-panel" @click.stop>
                    ...
                    <input
                        ...
                        @input="handleBorderContrastChange"
                        @change="flushBorderContrastChange"
                    >

                    <div class="section-title" style="margin-top: 16px; display: flex; justify-content: space-between; align-items: center;">
                        <span>自定义颜色</span>
                        <span v-if="hasCustomColors" class="color-reset" @click="resetCustomColors">恢复默认</span>
                    </div>
                    <div class="color-group">
                        <div class="color-group-label">主题色</div>
                        <div class="color-swatches">
                            <button v-for="c in primaryPresets" :key="c.color || '__default__'"
                                class="swatch-btn"
                                :class="{ active: (c.color || '') === (store.customPrimary || '') }"
                                @click="setPrimaryColor(c.color)">
                                <span v-if="!c.color" class="swatch-label">默认</span>
                                <span v-else class="swatch-dot" :style="{ background: c.color }"></span>
                            </button>
                            <label class="swatch-btn swatch-plus" title="自定义颜色">
                    ...
                    <div class="color-group">
                        <div class="color-group-label">边框色</div>
                        <div class="color-swatches">
                            <button v-for="c in borderPresets" :key="c.color || '__default__'"
                                class="swatch-btn"
                                :class="{ active: (c.color || '') === (store.customBorder || '') }"
...
const hasCustomColors = computed(() => !!(store.customPrimary || store.customBorder))
...
watch(() => store.backgroundOpacity, (newVal) => {
    backgroundOpacity.value = Number.isFinite(Number(newVal)) ? Number(newVal) : 1
})
watch(() => store.borderContrast, (newVal) => {
    borderContrast.value = Number.isFinite(Number(newVal)) ? Number(newVal) : 100
})
