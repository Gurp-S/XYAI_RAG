...
function startWatchingTask(taskId) {
  ...
  es.addEventListener('snapshot', (event) => {
    if (taskId !== activeTaskId || taskFinished) {
      es.close()
      return
    }
    try {
      ...
      if (status === 'SUCCESS') {
        ...
        taskFinished = true
        uploadStage.value = 'error'
        uploadStatusText.value = snapshot.displayText || snapshot.message || '处理失败'
        isUploading.value = false
        es.close()
        eventSource = null
      }
  ...
  es.addEventListener('error', () => {
    if (taskFinished || taskId !== activeTaskId) {
      es.close()
      eventSource = null
      return
    }
...
async function pollTaskStatus(taskId) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      uploadStatusText.value = `任务查询失败（重试 ${taskPollRetryCount}/${MAX_TASK_POLL_RETRIES}）`
    ...
    taskPollRetryCount += 1
    if (taskPollRetryCount > MAX_TASK_POLL_RETRIES) {
      stopWatchingWithError('任务状态查询失败，请稍后重试。')
      return
    }
...
async function startUpload() {
  ...
  xhr.onload = () => {
    ...
    try {
      ...
      clearPollingTimer()
      startWatchingTask(activeTaskId)
    } catch (error) {
      uploadStage.value = 'error'
