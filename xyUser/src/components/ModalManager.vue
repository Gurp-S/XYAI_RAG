...
/**
 * 使用 SSE 实时监听任务状态，SSE 不可用时降级为短轮询。
 */
function startWatchingTask(taskId) {
    currentTaskId = taskId
    taskFinished = false
    closeEventSource()
    clearPollingTimer()

    const es = new EventSource(`/upload/task/stream?taskId=${encodeURIComponent(taskId)}`)
    eventSource = es

    es.addEventListener('snapshot', (event) => {
        if (taskId !== currentTaskId || taskFinished) {
            es.close()
            return
        }
        try {
            ...
            if (status === 'SUCCESS') {
                taskFinished = true
                ui.uploadStatusText = snapshot.displayText || '上传完成，知识库已更新'
                ui.uploadProgress = typeof snapshot.progress === 'number' ? Math.min(100, Math.round(snapshot.progress)) : 100
                finishUpload()
                es.close()
                eventSource = null
            } else if (status === 'ERROR') {
                taskFinished = true
                ui.uploadStatusText = snapshot.displayText || snapshot.message || '处理失败'
                finishUpload()
                es.close()
                eventSource = null
            }
    ...
    es.addEventListener('error', () => {
        if (taskFinished || taskId !== currentTaskId) {
            es.close()
            eventSource = null
            return
        }
...
function stopWatchingWithError(message) {
    taskFinished = true
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (taskPollRetryCount > MAX_TASK_POLL_RETRIES) {
                stopWatchingWithError('任务查询失败，请稍后重试。')
                return
            }
            ui.uploadStatusText = `任务查询失败（重试 ${taskPollRetryCount}/${MAX_TASK_POLL_RETRIES}）`
        ...
        if (payload.code !== 200) {
            taskPollRetryCount += 1
            if (taskPollRetryCount > MAX_TASK_POLL_RETRIES) {
                stopWatchingWithError(payload.msg || '任务查询失败，请稍后重试。')
                return
            }
        ...
        scheduleTaskPoll(taskId, 1000)
    } catch (err) {
        if (taskFinished || taskId !== currentTaskId) return

        taskPollRetryCount += 1
        if (taskPollRetryCount > MAX_TASK_POLL_RETRIES) {
            stopWatchingWithError('任务状态查询失败，请检查服务后重试。')
            return
        }
...
function scheduleTaskPoll(taskId, delay = 1000) {
    if (taskFinished || taskId !== currentTaskId) return
