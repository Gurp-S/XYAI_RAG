<template>
  <section class="input-area">
    <div class="input-wrapper">
      <textarea :value="modelValue" @input="$emit('update:modelValue', $event.target.value)" @keydown.enter.exact.prevent="$emit('send')" placeholder="输入你的问题，按 Enter 发送，Shift+Enter 换行"></textarea>
      ...
      <button class="btn-send btn-send-inline"
              aria-label="发送消息"
              style="background: linear-gradient(135deg,#f47c8e,#ff9fb0); color:#fff; width:46px; height:46px; border-radius:18px; border:none; z-index:9999 !important; box-shadow:0 12px 24px rgba(244,124,142,0.18); pointer-events:auto;"
              @click="$emit('send')">
