...
|-----|----------------------------------------------------|-------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----|
| W01 | 存在明文 OSS key、DB 密码、LLM API key。                    | 立即轮换泄露密钥；本地文件仅保留占位；改为环境变量/密钥管理。                       | [application-local.yaml](main/resources/application-local.yaml)                                                                                                                                                                      | P0  |
| W02 | 使用 static USERID；大量接口直接收 userId 参数，存在越权风险。         | 移除 static 用户态；统一从鉴权上下文取 userId。                       | [UserController.java](com/XYai/myai/user/UserController.java)                                                                                                                                                                        | P0  |
...
| W23 | ensureReadyForRead 未加载时返回 null，语义弱。                | 返回显式状态对象（LOADED/UNLOADED/NOT_FOUND）。                  | [MilvusCollectionService.java](com/XYai/myai/rag/milvus/MilvusCollectionService.java)                                                                                                                                                | P1  |
...
| W31 | 登录仅靠 userId+password 表单，不使用 token/session。         | 改为标准登录票据（JWT/Session）+ 自动刷新策略。  0       1             | [Login.vue](../frontend/src/components/Login.vue)                                                                                                                                                                                    | P0  |
...
| T29 | [LLMMCPParameterExtractor.java](com/XYai/myai/rag/mcp/tools/LLMMCPParameterExtractor.java)                                                           | 参数抽取能力落地           | 1) 接 LLM 输出 JSON schema 2) 校验 required 字段 3) 失败回退人工参数                                    | MCP 自动参数提取可用    |
